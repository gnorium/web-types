...
/// SVGContent Path container
public enum SVGPath {
  /// SVGContent Path Definition (d attribute)
  public enum Definition {
    ...
    public enum Command: Sendable {
      ...
      case cubicBezier(
        x1: Double, y1: Double, x2: Double, y2: Double, x: Double, y: Double, relative: Bool)
      ...
      case arc(
        rx: Double, ry: Double, rotation: Double, largeArc: Int, sweep: Int, x: Double, y: Double,
        relative: Bool)
      case close
      ...
      public var pathString: String {
        ...
        case .arc(let rx, let ry, let rotation, let largeArc, let sweep, let x, let y, let relative):
          ...
          return
            "\(cmd) \(doubleToString(rx)),\(doubleToString(ry)) \(doubleToString(rotation)) \(largeArc) \(sweep) \(doubleToString(x)),\(doubleToString(y))"
        case .close:
          return "Z"
        }
      }
...
public func C(_ x1: Double, _ y1: Double, _ x2: Double, _ y2: Double, _ x: Double, _ y: Double)
  -> SVGPath.Definition.Command
{
...
public func A(
  _ rx: Double, _ ry: Double, _ rotation: Double, _ largeArc: Int, _ sweep: Int, _ x: Double,
  _ y: Double
) -> SVGPath.Definition.Command {
  .arc(
    rx: rx, ry: ry, rotation: rotation, largeArc: largeArc, sweep: sweep, x: x, y: y,
    relative: false)
}

public func A(
  _ rx: Double, _ ry: Double, _ rotation: Double, _ largeArc: Bool, _ sweep: Bool, _ x: Double,
  _ y: Double
) -> SVGPath.Definition.Command {
  .arc(
    rx: rx, ry: ry, rotation: rotation, largeArc: largeArc ? 1 : 0, sweep: sweep ? 1 : 0, x: x,
    y: y, relative: false)
}
...
public func a(
  ...
  .arc(
    rx: rx, ry: ry, rotation: rotation, largeArc: largeArc, sweep: sweep, x: x, y: y, relative: true
  )
}
